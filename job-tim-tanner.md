...
- Model: **Nano Banana 2** or **GPT Image 2**, 2K, web app.
- **No Reference Element attached.** There is nothing to attach yet — this batch *creates*
  the source material the Element will later be built from.
- Generate **3–4 variants of every one of the 21 shots below**, not 2–3. With no photo
  anchoring the face, drift across the batch is the main risk, and a heavy-set build
  drifting thinner is the specific failure to watch for on every single variant.
